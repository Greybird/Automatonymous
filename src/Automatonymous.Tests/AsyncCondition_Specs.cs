...
namespace Automatonymous.Tests
{
    ...
    [TestFixture]
    public class Using_an_asynccondition_in_a_state_machine
    {
        [Test]
            ...
            public InstanceStateMachine()
            {
                During(Initial,
                    When(Started)
                        ...
                        .IfAsync(async context => await Task.FromResult(context.Data.InitializeOnly), x => x.Then(context => Console.WriteLine("Initializing Only!")))
                        .TransitionTo(Initialized));

                During(Initial,
                    When(ExplicitFilterStarted, context => true)
                        .IfAsync(async context => await Task.FromResult(false), binder => binder
                            .Then(context => Console.WriteLine("Should not be here!"))
                            .TransitionTo(ShouldNotBeHere))
                        .IfAsync(async context => await Task.FromResult(true), binder => binder.Then(context => Console.WriteLine("Initializing Only!"))));

                During(Running,
                    When(Finish)
                        .Finalize());

                WhenEnter(Initialized, x => x.IfAsync(async context => await Task.FromResult(!context.Instance.InitializeOnly), b => b.TransitionTo(Running)));
            }
